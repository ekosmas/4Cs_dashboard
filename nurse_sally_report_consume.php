...
$PAGE->set_url('/blocks/enhance/nurse_sally_report_consume.php', array('id' => $courseid));
$PAGE->set_pagelayout('standard');
$PAGE->set_heading(get_string('nurse_sally_report_consume', 'block_enhance'));

$settingsnode = $PAGE->settingsnav->add(get_string('enhancesettings', 'block_enhance'));
$sectionsurl = new moodle_url('/blocks/enhance/nurse_sally_report_consume.php', array('id' => $id, 'courseid' => $courseid, 'blockid' => $blockid));
$editnode = $settingsnode->add(get_string('nurse_sally_report_consume_nav', 'block_enhance'), $sectionsurl);
	...
	if ($bool_chart_empty) {
		echo 
			'<div style="width: 40%; height: 40%">'.
				...
				'<p style = "text-align: center">There are no events for activities or resources of type consume, for this course.</p>'.
			'</div>';
	}
	else {
		echo 
