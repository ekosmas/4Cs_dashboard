...
$PAGE->set_url('/blocks/enhance/my_report_consume.php', array('id' => $courseid));
$PAGE->set_pagelayout('standard');
$PAGE->set_heading(get_string('my_report_consume', 'block_enhance'));

$settingsnode = $PAGE->settingsnav->add(get_string('enhancesettings', 'block_enhance'));
$sectionsurl = new moodle_url('/blocks/enhance/my_report_consume.php', array('id' => $id, 'courseid' => $courseid, 'blockid' => $blockid));
$editnode = $settingsnode->add(get_string('my_report_consume_nav', 'block_enhance'), $sectionsurl);
...
if (!has_capability('block/enhance:earnpoints', $context, NULL, false)) {
	...
	$events_mandatory = $DB->get_records_sql('
		...
		array( 'tval' => 'MANDATORY', 'type' => 'CONSUME')
	);
	...
	$events_optional = $DB->get_records_sql('
		...
		array( 'tval' => 'OPTIONAL', 'type' => 'CONSUME')
	);
		...
		echo 
			'<div style="width: 40%; height: 40%">'.
				'<h2 style = "text-align: center">Mandatory</h2>
				 <p style = "text-align: center">Consume</p>'.
				'<p style = "text-align: center">You have no mandatory events for activities or resources of type consume.</p>'.
				'<hr>';
	}
	else {
		echo 
	...
	if ($bool_optional_chart_empty) {
		echo 
				'<h2 style = "text-align: center">Optional</h2>
				 <p style = "text-align: center">Consume</p>'.
				 '<p style = "text-align: center">You have no optional events for activities or resources of type consume.</p>'.
			'</div>';
	}
	else {
		echo 
